...
namespace CefiBrowser
{
    ...
    internal class MVerifyCode
    {
        ...
        public bool reOCR { set; get; }
        public MVerifyCode(Bitmap pic)
        {
        ...
        /// <summary>
        /// 反正回GetPicValidByValue处理后的图片，但是调用只能在GetPicValidByValue后面
        /// </summary>
        /// <returns></returns>
        public Bitmap getGBitmap()
        {
            return bmpobj;
        }

        public Bitmap GetCPicValidByValue(int dgGrayValue, int CharsCount, Bitmap Ybitmap)
        {

            int posx1 = Ybitmap.Width; int posy1 = Ybitmap.Height;
            int posx2 = 0; int posy2 = 0;
            for (int i = 0; i < Ybitmap.Height; i++)  //找有效区
            {
                for (int j = 0; j < Ybitmap.Width; j++)
                {
                    int pixelValue = Ybitmap.GetPixel(j, i).R;
                    if (pixelValue < dgGrayValue)  //根据灰度值
                    {
                        if (posx1 > j) posx1 = j;
                        if (posy1 > i) posy1 = i;
            ...
            if (Span < CharsCount)
            {
                ...
                if (posx1 > leftSpan)
                    posx1 = posx1 - leftSpan;
                if (posx2 + Span - leftSpan < Ybitmap.Width)
                    posx2 = posx2 + Span - leftSpan;
            }
            //复制新图
            Rectangle cloneRect = new Rectangle(posx1, posy1, posx2 - posx1 + 1, posy2 - posy1 + 1);
            Bitmap Jbitmap = Ybitmap.Clone(cloneRect, Ybitmap.PixelFormat);
            return Jbitmap;
        }
        ...
        public Bitmap[] GetSplitPics(int RowNum)
        {
            ...
            //    }
            //}
            int singW = 9; //按固定宽度，这里是一个数字的宽度为9
            int singH = bmpobj.Height;
            Bitmap[] PicArray = new Bitmap[4 * 1];
            Bitmap[] PP = new Bitmap[1];
            Rectangle cloneRect;
            Rectangle NcloneRect;
            cloneRect = new Rectangle(0, 0, singW, singH);
            NcloneRect = new Rectangle(singW, 0, bmpobj.Width - 9, bmpobj.Height);

            PicArray[0] = bmpobj.Clone(cloneRect, bmpobj.PixelFormat);//复制小块图
            PP[0] = bmpobj.Clone(NcloneRect, bmpobj.PixelFormat);

            PP[0] = GetPicValidByValue_P(128, 3, PP[0]);
            cloneRect = new Rectangle(0, 0, singW, singH);
            NcloneRect = new Rectangle(singW, 0, PP[0].Width - 9, PP[0].Height);
            PicArray[1] = PP[0].Clone(cloneRect, PP[0].PixelFormat);//复制小块图
            if (GetCPicValidByValue(128, 1, PicArray[1]).Width < 9)
            {
                cloneRect = new Rectangle(0, 0, singW + 1, singH);
                NcloneRect = new Rectangle(singW + 1, 0, PP[0].Width - singW - 1, PP[0].Height);
                PicArray[1] = PP[0].Clone(cloneRect, PP[0].PixelFormat);//复制小块图
            }
            PP[0] = PP[0].Clone(NcloneRect, PP[0].PixelFormat);

            PP[0] = GetPicValidByValue_P(128, 2, PP[0]);
            cloneRect = new Rectangle(0, 0, singW, singH);
            NcloneRect = new Rectangle(singW, 0, PP[0].Width - 9, PP[0].Height);
            PicArray[2] = PP[0].Clone(cloneRect, PP[0].PixelFormat);//复制小块图
            if (GetCPicValidByValue(128, 1, PicArray[2]).Width < 9)
            {
                cloneRect = new Rectangle(0, 0, singW + 1, singH);
                NcloneRect = new Rectangle(singW + 1, 0, PP[0].Width - singW - 1, PP[0].Height);
                PicArray[2] = PP[0].Clone(cloneRect, PP[0].PixelFormat);//复制小块图
            ...
            PicArray[3] = GetPicValidByValue_P(128, 1, PP[0]);
        ...
          /// <param name="CharsCount">有效字符数</param>
          /// <returns></returns>
        public Bitmap GetPicValidByValue_P(int dgGrayValue, int CharsCount, Bitmap pbmpobj)
        {

            int posx1 = pbmpobj.Width; int posy1 = pbmpobj.Height;
            int posx2 = 0; int posy2 = 0;
            for (int i = 0; i < pbmpobj.Height; i++)  //找有效区
            {
                for (int j = 0; j < pbmpobj.Width; j++)
                {
                    int pixelValue = pbmpobj.GetPixel(j, i).R;
                    if (pixelValue < dgGrayValue)  //根据灰度值
                    {
                        if (posx1 > j) posx1 = j;
                        if (posy1 > i) posy1 = i;
            ...
            if (Span < CharsCount)
            {
                ...
                if (posx1 > leftSpan)
                    posx1 = posx1 - leftSpan;
                if (posx2 + Span - leftSpan < pbmpobj.Width)
                    posx2 = posx2 + Span - leftSpan;
            }
            //复制新图
            Rectangle cloneRect = new Rectangle(posx1, posy1, posx2 - posx1 + 1, posy2 - posy1 + 1);
            pbmpobj = pbmpobj.Clone(cloneRect, pbmpobj.PixelFormat);
            return pbmpobj;
        }
